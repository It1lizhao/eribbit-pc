...
export default {
  ...
  setup() {
    ...
    const filterData = ref(null) // 数据
    const filterLoading = ref(false) // 控制骨架显示
        ...
        if (newVal && route.path === '/category/sub/' + newVal) {
          ...
          newVal &&
            findSubCategoryFilter(route.params.id).then(({ result }) => {
              // 品牌全部
              result.selectedBrand = null // 加入一个属性 通过点击 传入ID 控制当前字体高亮样式
              result.brands.unshift({ id: null, name: '全部' })
              // 销售属性全部
              result.saleProperties.forEach((p) => {
                p.selectedProp = null // 加入一个属性 通过点击 传入ID 控制当前字体高亮样式
